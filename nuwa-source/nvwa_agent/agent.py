...
def create_nvwa_agent(
    ...
    # Check if nuwa-agent-skill directory exists
    if not skill_source_path.exists():
        print(f"⚠️  Warning: nuwa-agent-skill directory not found at {skill_source_path}")
        print("   The agent will work but won't have access to nuwa-skill functionality.")
        skills_list = None
        backend = None
    else:
        # Source: deepagents/middleware/skills.py:404-479
        # Skills are loaded from source directories. The source path should point
        # to the directory CONTAINING the skill directories, not the skill itself.
        #
        # Expected structure:
        #   nuwa-agent-skill/    ← source directory
        #   └── nuwa/            ← skill directory (name matches SKILL.md)
        #       └── SKILL.md
        #
        # Configuration:
        # - backend root_dir = project_root (parent of nuwa-agent-skill)
        # - skills = ["nuwa-agent-skill"] (path to source directory)
        #
        # This way, backend will find nuwa-agent-skill/nuwa/SKILL.md
        skills_list = ["nuwa-agent-skill"]
        backend = FilesystemBackend(root_dir=str(project_root), virtual_mode=True)
        print(f"✅ Loading skills from nuwa-agent-skill/ (contains nuwa skill)")
