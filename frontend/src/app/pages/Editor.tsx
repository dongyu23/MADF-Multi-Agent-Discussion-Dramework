...
import { ArrowLeft, Save, FileText, Folder, Terminal, Loader2, CheckCircle2, Zap, Search, Box, Copy, Cpu } from "lucide-react";
...
interface LogEvent {
  level: "idle" | "main" | "sub" | "tool" | "done" | "error";
  message: string;
  extra?: any;
}
...
export function Editor() {
  ...
  const generationDone = character?.status === "ready" || logs.some(l => l.level === "done");
        ...
        <div className="flex-1 flex bg-white relative min-w-0">
            ...
            {files.length === 0 && isGenerating ? (
              ...
              <Suspense fallback={<div className="flex items-center justify-center h-full"><Loader2 className="animate-spin" size={24} /></div>}>
                <MonacoEditor height="100%" defaultLanguage="markdown" value={code}
                  onChange={() => {}} options={{ readOnly: true, minimap: { enabled: false }, fontSize: 14, fontFamily: "'JetBrains Mono', 'Fira Code', monospace", wordWrap: "on", padding: { top: 24, bottom: 24 }, lineHeight: 1.6 }} />
              </Suspense>
            )}
          </div>
          ...
          {!isViewMode && (isGenerating || logs.length > 0) && (
            <div className="w-[360px] bg-slate-950 border-l border-slate-800 flex flex-col flex-shrink-0 z-10 text-slate-300 relative overflow-hidden">
              ...
              {/* Header */}
              <div className="h-14 border-b border-slate-800/60 flex items-center px-5 justify-between bg-slate-950/80 backdrop-blur-sm z-10 relative flex-shrink-0">
                ...
                <div className="flex items-center gap-2 text-xs font-mono">
                  {generationDone ? (
                    <span className="text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded-full border border-emerald-400/20 flex items-center gap-1">
                      <CheckCircle2 size={12} /> 已完成
                    </span>
                  ) : generationFailed ? (
                    <span className="text-red-400 bg-red-400/10 px-2.5 py-1 rounded-full border border-red-400/20">失败</span>
                  ) : (
                    <span className="text-amber-500/80 bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20 flex items-center gap-1">
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 font-mono text-xs z-10 relative" ref={scrollRef}>
                ...
                <AnimatePresence>
                  {logs.map((log, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, y: 10, filter: "blur(4px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.3 }}
